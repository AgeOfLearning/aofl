...
function refreshTests(context) {
    ...
    context._testRunners.forEach(function (tr) {
        ...
        const webdriverCapabilities = _.clone(tr.def);
        ...
        if (tr.sessionId) {
            tr.resetDone();
            promises.push(tr.donePromise);
            tr.browser.attach(tr.sessionId, (error) => {
                tr._init(error, tr.sessionId);
            });
        }
        else {
            throw new Error('No active sessions. Restart tests.');
        }
    });
    return Promise.all(promises);
}
exports.refreshTests = refreshTests;
