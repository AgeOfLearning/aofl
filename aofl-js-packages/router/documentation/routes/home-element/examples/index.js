...
class Navigation extends AoflElement {
  _render() {
    return super._render((context, html) => html`
      ...
      <a href="" onclick="${(e) => {e.preventDefault(); router.navigate('/members', false, true)}}">Members page</a>
    `);
  }
}
    ...
    router.after((request, response, next) => {
      if (response.matchedRoute === null) {
        ...
        .then((component) => {
          ...
          let props = '';
          for (let prop in response.matchedRoute.props) {
            if (!response.matchedRoute.props.hasOwnProperty(prop)) continue;
            props += prop + '="' + response.matchedRoute.props[prop] + '" ';
          }
          let t = `<${component.is} ${props}></${component.is}>`;
